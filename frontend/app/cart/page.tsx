...
const page = () => {
  ...
  const cartIds=currentUser.cart
  var cart:any[] = []
  
  useEffect(()=>{
    cartIds.map((e: any)=>{
      axios.get(`http://localhost:3001/items/cartItems/${e}`)
      .then(item=>{cart=[...cart,item.data];console.log(cart)})
    })
  },[])
    ...
    <div>
      ...
      <div id="cart-container">
        ...
        <div id="cart-image">
          ...
          {currentUser.cart.length!==0 && <div> hello</div>}
        </div>
    </div>
  );
};
